...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        int cantidadPuestosCentro = 0;
        int cantidadPuestosNorte = 0;
        int cantidadPuestosSur = 0;
        int cantidadPuestosOccidente = 0;
        int cantidadPuestosOriente =0;
        int cantidadPuestosTotales = 0;
        String NombreZona = "";

        for (Puesto p: puestos){
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_CENTRO)){
                cantidadPuestosCentro++;
            }
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_NORTE)){
                cantidadPuestosNorte++;
            }
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_SUR)){
                cantidadPuestosSur++;
            }
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_OCCIDENTE)){
                cantidadPuestosOccidente++;
            }
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_ORIENTE)){
                cantidadPuestosOriente++;
            }
        }

        cantidadPuestosTotales = cantidadPuestosCentro;
        NombreZona = "Zona Centro";
        if (cantidadPuestosTotales < cantidadPuestosNorte){
            NombreZona = "Zona Norte";
            cantidadPuestosTotales = cantidadPuestosNorte;
        }
        if (cantidadPuestosTotales < cantidadPuestosSur){
            NombreZona = "Zona Sur";
            cantidadPuestosTotales = cantidadPuestosSur;
        }
        if (cantidadPuestosTotales < cantidadPuestosOccidente){
            NombreZona = "Zona Occidente";
            cantidadPuestosTotales = cantidadPuestosOccidente;
        }
        if (cantidadPuestosTotales < cantidadPuestosOriente){
            NombreZona = "Zona Oriente";
            cantidadPuestosTotales = cantidadPuestosOriente;
        }
        return "Hay " + cantidadPuestosTotales + " puestos ocupados en la " + NombreZona + ". ";
    }
    ...
    public String metodo2() {
        double porcentaje = this.porcentajeOcupacion();

        if (porcentaje < 60){
            return " No se ha alcanzado el punto de equilibrio. ";
        }
        else{
            return " Ya se alcanzo el punto de equilibrio. ";
        }
    }
